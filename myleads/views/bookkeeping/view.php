...
<div class="bookkeeping-wrap">
    <?php if($info['have_hosting']): ?>
        ...
        <div class="hosting-details focus-box book-box">
            ...
            <?php else: ?>
                    ...
                    <?= $info['hostPriceMon'] ?>₪ + מע"מ
                    <br/>
                    <b>
                        מחיר אחסון שנתי: 
                    </b>
                    <?= $info['hostPriceYear'] ?>₪ (כולל מע"מ)
    ...
    <?php if($info['have_domain']): ?>
        
        <div class="domain-details focus-box book-box">
            <b>
                תאריך תוקף דומיין: 
            </b>
            <?= hebdt($info['domainEndDate'],"d-m-Y") ?>
            <br/>
                    ...
                    <li>
                        ...
                        <div class="booking-pay-form">
                            ...
                            <form action='bookkeeping/send_to_yaad_domain/' method='post' name='sendto_Bookkeeping' id='sendto_yaad_domain_form'>	
                                <div class='buy_domain_desc form-group'>
                                    ...
                                    <?php else: ?>
                                        <div class='buy_leads_token_select form-group '>
                                            <label for='use_token'>בחר כרטיס אשראי</label>
                                            <select onchange="toggle_token_fields(this)" data-token_fields="token-fields-domain" data-token_submit="domain-form-submit" name='use_token' id='use_token_select' class="form-select use-token input_style">
                                                <option value='0'>השתמש בכרטיס חדש</option>
                                                <?php foreach($info['user_cc_tokens'] as $token): ?>
                                                    <option value='<?= $token['L4digit'] ?>'><?= $token['L4digit'] ?>**** **** **** </option>		
                                                <?php endforeach; ?>
                                    ...
                                    <?php endif; ?>
                                        <div class="token-fields-domain hidden">
                                        ...
                                        <input type='submit' class='submit_style domain-form-submit' value='עבור לטופס תשלום מאובטח' data-no_token_text = 'עבור לטופס תשלום מאובטח' data-use_token_text = 'בצע תשלום'  />
                                </div>
                            </form>
                        </div>   
                    </li>
