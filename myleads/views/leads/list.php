...
<div ng-app="leadsApp" ng-controller="leadsCtrl">
		...
		<div id="filter_form_wrap" class="filter-form-wrap span2"> 
			...
			<div id="leads_filter_form_wrap">
				...
				<form id="leads_filter_form" class="filter-form" action="" method="POST">
					<?php if($this->data['show_row']): ?>
						<input id="filter_auto_show_row" type="hidden" name="show_row" value="<?php echo $this->data['show_row']; ?>" />
					<?php endif; ?>
							...
							<div class="select_wrap">								
								<select class="form-input" name="leads_filter[cat]" id="cat_select">
									<?php foreach($filter['cat_options'] as $option): ?>
										<option value="<?php echo $option['id']; ?>" <?php echo $option['selected']; ?>><?php echo $option['label']; ?></option>
									<?php endforeach; ?>
			...
			<div ng-show="!lead_data" id="leads_list_view">
				<div class="leads-list" ng-if="pagesData.lead_count > 0">
					<div id="toolbar">
						...
						<div id="task_tools">
							<div class="dropdown show lead_credits_menu_wrap">
							  <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								<span class="lead_credits_menu leads-header-link">{{user.lead_credit}}</span>
							  </a>
					...
					<div id="listTable_wrap" class="scroll">
						<div id="listTable_header_wrap">
							<div>
								<a href="<?= inner_url('deals_closed_payments/report/') ?>" title="צפה בתשלומים על הכנסות">
								...
								<table id="listTable" class="col-md-12 table-bordered table-striped table-condensed cf">
									...
									<tbody>
										...
										<tr class="responsive-small view_state_{{lead.estimate_form_data['view_state']}} refund_request_sent_{{lead.estimate_form_data['refund_request_sent']}} status_{{lead.estimate_form_data['status']}} tag-color-{{lead.estimate_form_data['tag_color']}}" ng-repeat="lead in leadsList" data-lead_id="{{lead.estimate_form_data['row_id']}}" ng-click="show_lead($event);">
											...
											<td data-title="{{lead.estimate_form_data['tag_str']}}">{{lead.estimate_form_data['full_name']}}</td>
											...
											<td data-title="" class="responsive-closed responsive-full-row">{{lead.estimate_form_data['note']}}</td>
			...
			<div ng-if="lead_data" id="lead_edit_view">
				<div>
				  ...
				  <ul class="nav nav-tabs" role="tablist" id="lead_data_tabs">
					...
					<li ng-if="user['access_refunds']=='1' && lead_data['open_state'] == '1'" role="presentation"><a href="#lead_refund" aria-controls="lead_refund" role="tab" data-toggle="tab">זיכוי</a></li>
				  ...
				  <div class="tab-content">
					<div role="tabpanel" class="tab-pane active" id="lead_details">
						...
						<div id="details_form_wrap">
							<div class="row-fluid">
								<div ng-if="lead_data['open_state'] == '0'" class='span3 lead_form_btn form-group'>
									<label for="lead_form_buy">הליד סגור</label>
									<div ng-if="user.has_special_closed_lead_alert == '1'" class='' style='color:red;'>
										<b>שים לב.</b><br/>
	...
	`								</div>										
								...
								<div class="lead-form-content">
									<div class="row-fluid">
										...
										<div class='span3 lead_form_item form-group'>
											<label for="data_arr[full_name]">שם</label>
											<input type="text" ng-model="lead_data['full_name']" name="data_arr[full_name]" value="" class="input_style text-input name" />
										</div>
										...
										<div ng-if="lead_data['open_state'] == '1'" class='span3 lead_form_item form-group'>
											<label for="data_arr[phone]">טלפון</label>
											...
											<a href="whatsapp://send?text=text-normal&phone=+972722706395">
												chat_whatsapp
											</a>
										</div>	
										<div ng-if="lead_data['open_state'] == '0'" class='span3 lead_form_item form-group'>
									...
									<div class="row-fluid">										
										...
										<div class='span6 lead_form_item form-group'>
											<label for="data_arr[note]">הערות</label>
											<textarea id="lead_note_textarea" ng-model="lead_data['note_full']" name="data_arr[note]" class="input_style textarea content"></textarea>
										</div>	
					...
					<div ng-if="user['access_refunds']=='1'" role="tabpanel" class="tab-pane" id="lead_refund">
						<div class='refund_request_button_wrap form-group'>
							<form action="javascript://" id="refund_request_form" class="refund_form">
								...
								<div ng-if="lead_data['no_refund_reason'] == 'doubled'">
									...
									<br/>
									<span data-lead_id="{{lead_data['id']}}">
										<button data-lead_id="{{lead_data['duplicate_id']}}" type="button"  ng-click="show_lead_respo($event);" style="width:auto;">לחץ כאן לראות את הליד שחוייב</button>
									</span>
								</div>									
