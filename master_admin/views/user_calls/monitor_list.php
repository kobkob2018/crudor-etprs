<table id="current_phone_calls_return">
    <?php foreach($info['phone_list'] as $phone_data): ?>
        <tr  class="phone_list_row" data-state="phone" data-uniqid="<?php echo $phone_data['id']; ?>" >
            ...
            <td class="current_call_td_from"><a target="_BLANK" href="<?= get_config('master_url') ?>/?aff_id=18&custom_phone=<?php echo $phone_data['call_from']; ?>&custom_cat=<?php echo $phone_data['custom_cat']; ?>&link_uniq=<?php echo $phone_data['id']; ?>"><?php echo $phone_data['call_from']; ?></a></td>
