<?php
  class Migration_user_paymentsController extends CrudController{
    public $add_models = array("users","migration_user", "migration_user_payments");
    ...
    public function prepare(){
      ...
      $info = array(
        'migration_exist'=>Migration_user_payments::check_if_migration_exist($filter_arr)
      );
      return $this->include_view("migration_user_payments/prepare.php",$info);
    }
    ...
    public function do_migrate(){
        $filter_arr = $this->get_base_filter();
        $migration_user = Migration_user::find($filter_arr);      
        Migration_user_payments::do_migrate($_REQUEST['user_id'],$migration_user);
        ...
        return $this->redirect_to(inner_url("migration_user_payments/prepare/?user_id=".$_REQUEST['user_id']));
    }


    public function delete_older(){   
        Migration_user_payments::delete_older($_REQUEST['user_id']);
