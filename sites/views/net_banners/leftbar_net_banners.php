<?php foreach($info['net_banners'] as $banner): ?>  
    <div class="leftbar-net-banner net-banner leftbar-item counted-module" data-link="<?= add_url_params($banner['goto_href'],array('banner_id'=>$banner['id'])) ?>" data-count_url="<?= inner_url("banner_count/clicks/") ?>?banner_id=<?= $banner['id'] ?>">
        <img src="<?= inner_url("banner_count/views/") ?>?banner_id=<?= $banner['id'] ?>" style="display:none" />
        <?php if($banner['video'] != ''): ?>
            <a href = "javascript://" class="count-clicker">
                <video class='cube-banner-vid' width="100%" autoplay loop muted="" playsinline <?php if($banner['image'] != ""): ?> poster="<?= $this->file_master_url_of('net_banners', $banner['image']) ?>" <?php endif; ?>>
