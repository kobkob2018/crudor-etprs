<?php
  class Product_sub extends TableModel{
    ...
    public static function get_fields_collection_for_cat_delete($delete_cat_id){
        ...
        $sql = "SELECT id, label FROM product_cat WHERE id != :delete_cat_id";
        $db = Db::getInstance();		
        $req = $db->prepare($sql);
    ...
    public static function move_products_from_cat_to($cat_from,$cat_to){
        $execute_arr = array(
            'cat_from'=>$cat_from,
            'cat_to'=>$cat_to
        );
        $sql = "UPDATE products SET cat_id = :cat_to WHERE cat_id = :cat_from";
        $db = Db::getInstance();		
        $req = $db->prepare($sql);
        $req->execute($execute_arr);
    }
